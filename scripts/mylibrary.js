let currentFilter = '';

document.addEventListener('DOMContentLoaded', () => {
    if (!userManager.redirectIfNotLoggedIn()) return;
    
    displayLibrary();
    setupFilters();
    setupSort();
    updateStats();
});

function displayLibrary() {
    const library = libraryManager.getLibrary();
    const grid = document.getElementById('booksGrid');
    const emptyState = document.getElementById('emptyState');
    
    let booksToDisplay = library;
    if (currentFilter) {
        booksToDisplay = library.filter(item => item.status === currentFilter);
    }

    if (booksToDisplay.length === 0) {
        grid.style.display = 'none';
        emptyState.classList.add('show');
    } else {
        grid.style.display = 'grid';
        emptyState.classList.remove('show');
        grid.innerHTML = '';

        booksToDisplay.forEach(item => {
            const card = document.createElement('div');
            card.className = 'book-item';
            
            const statusClass = item.status.toLowerCase().replace(/ /g, '-');
            
            card.innerHTML = `
                <img src="${item.image}" alt="${item.title}" class="book-cover" onclick="window.location.href='book-detail.html?id=' + ${getBookById(item.bookId).id}">
                <div class="book-details">
                    <div class="title">${item.title}</div>
                    <div class="author">${item.author}</div>
                    <span class="status-badge ${statusClass}">${item.status}</span>
                    <div class="actions">
                        <button class="btn-small btn-update" onclick="editBook(${item.bookId})">Edit</button>
                        <button class="btn-small btn-remove" onclick="removeBook(${item.bookId})">Remove</button>
                    </div>
                </div>
            `;
            grid.appendChild(card);
        });
    }
}

function setupFilters() {
    const filterBtns = document.querySelectorAll('.filter-btn');
    filterBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            filterBtns.forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            currentFilter = btn.dataset.status;
            displayLibrary();
            updateStats();
        });
    });
}

function setupSort() {
    const sortSelect = document.getElementById('sortSelect');
    sortSelect.addEventListener('change', (e) => {
        const library = libraryManager.getLibrary();
        let sorted = [...library];
        
        if (currentFilter) {
            sorted = sorted.filter(item => item.status === currentFilter);
        }

        switch(e.target.value) {
            case 'A-Z':
                sorted.sort((a, b) => a.title.localeCompare(b.title));
                break;
            case 'Title':
                sorted.sort((a, b) => a.title.localeCompare(b.title));
                break;
            default:
                sorted.reverse();
        }

        const grid = document.getElementById('booksGrid');
        grid.innerHTML = '';
        
        sorted.forEach(item => {
            const card = document.createElement('div');
            card.className = 'book-item';
            const statusClass = item.status.toLowerCase().replace(/ /g, '-');
            
            card.innerHTML = `
                <img src="${item.image}" alt="${item.title}" class="book-cover">
                <div class="book-details">
                    <div class="title">${item.title}</div>
                    <div class="author">${item.author}</div>
                    <span class="status-badge ${statusClass}">${item.status}</span>
                    <div class="actions">
                        <button class="btn-small btn-update" onclick="editBook(${item.bookId})">Edit</button>
                        <button class="btn-small btn-remove" onclick="removeBook(${item.bookId})">Remove</button>
                    </div>
                </div>
            `;
            grid.appendChild(card);
        });
    });
}

function updateStats() {
    const library = libraryManager.getLibrary();
    const wantToRead = library.filter(b => b.status === 'Want to Read').length;
    const reading = library.filter(b => b.status === 'Reading').length;
    const finished = library.filter(b => b.status === 'Finished').length;
    
    document.getElementById('libraryStats').textContent = 
        `Total: ${library.length} | Want to Read: ${wantToRead} | Reading: ${reading} | Finished: ${finished}`;
}

function editBook(bookId) {
    const status = prompt('Select status:\n1. Want to Read\n2. Reading\n3. Finished');
    let newStatus = '';
    
    switch(status) {
        case '1':
            newStatus = 'Want to Read';
            break;
        case '2':
            newStatus = 'Reading';
            break;
        case '3':
            newStatus = 'Finished';
            break;
        default:
            return;
    }
    
    const result = libraryManager.updateBookStatus(bookId, newStatus);
    if (result.success) {
        showNotification('Book updated!', 'success');
        displayLibrary();
        updateStats();
    }
}

function removeBook(bookId) {
    if (confirm('Remove this book from your library?')) {
        const result = libraryManager.removeBookFromLibrary(bookId);
        if (result.success) {
            showNotification('Book removed', 'success');
            displayLibrary();
            updateStats();
        }
    }
}

function showNotification(message, type) {
    const notification = document.getElementById('notification');
    notification.textContent = message;
    notification.className = `notification show ${type}`;

    setTimeout(() => {
        notification.classList.remove('show');
    }, 3000);
}

function setupSearch() {
    const searchInput = document.querySelector('.search input');
    const searchBtn = document.querySelector('.search button');

    if (searchInput) {
        searchInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                performSearch();
            }
        });
    }

    if (searchBtn) {
        searchBtn.addEventListener('click', performSearch);
    }
}

function performSearch() {
    const searchInput = document.querySelector('.search input');
    const query = searchInput ? searchInput.value.trim() : '';
    
    if (query.length > 0) {
        showSearchResults(query);
    }
}

function showSearchResults(query) {
    const results = searchBooks(query);
    
    if (results.length === 0) {
        alert('No books found matching: ' + query);
        return;
    }

    if (results.length === 1) {
        window.location.href = `book-detail.html?id=${results[0].id}`;
    } else {
        window.location.href = `search-results.html?query=${encodeURIComponent(query)}`;
    }
}

function setupCategoryFilter() {
    const categorySelects = document.querySelectorAll('.categories select');
    const searchSelects = document.querySelectorAll('.search select');

    const allSelects = [...categorySelects, ...searchSelects];

    allSelects.forEach(select => {
        const categories = getCategories();
        select.innerHTML = '<option value="">All Categories</option>';
        
        categories.forEach(cat => {
            const option = document.createElement('option');
            option.value = cat;
            option.textContent = cat;
            select.appendChild(option);
        });

        select.addEventListener('change', (e) => {
            const selectedCategory = e.target.value;
            if (selectedCategory) {
                filterByCategory(selectedCategory);
            }
        });
    });
}

function filterByCategory(category) {
    const books = getBooksByCategory(category);
    
    if (books.length === 0) {
        alert('No books in this category');
        return;
    }

    window.location.href = `category-books.html?category=${encodeURIComponent(category)}`;
}
function updateUserIcon() {
    const userIcon = document.getElementById('userIcon');
    const isLoggedIn = userManager.isUserLoggedIn();
    const currentUser = userManager.getCurrentUser();

    if (isLoggedIn && currentUser) {
        const userName = currentUser.name;
        userIcon.innerHTML = `
            <a href="mylibrary.html" title="${userName}">
                <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="100" cy="60" r="35" fill="#0b7c6b"/>
                    <path d="M30 170 C30 130 170 130 170 170 L170 190 L30 190 Z" fill="#0b7c6b"/>
                </svg>
            </a>
            <div class="user-menu">
                <p>${userName}</p>
                <button onclick="userManager.logout(); window.location.href='home.html';">Logout</button>
            </div>
        `;
    } else {
        userIcon.innerHTML = `<a href="login.html">
            <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
                <circle cx="100" cy="60" r="35" fill="#000"/>
                <path d="M30 170 C30 130 170 130 170 170 L170 190 L30 190 Z" fill="#000"/>
            </svg>
        </a>`;
    }
}

document.addEventListener('mouseover', (e) => {
    if (e.target.closest('#userIcon')) {
        const menu = document.querySelector('.user-menu');
        if (menu) menu.style.display = 'block';
    }
});

document.addEventListener('mouseout', (e) => {
    if (e.target.closest('#userIcon')) {
        const menu = document.querySelector('.user-menu');
        if (menu) menu.style.display = 'none';
    }
});



document.addEventListener('DOMContentLoaded', setActiveLink);
function updateActiveNavLink() {
    const currentUrl = window.location.href;
    
    const navLinks = document.querySelectorAll('.links li a');

    navLinks.forEach(link => {
        link.parentElement.classList.remove('active');

        if (currentUrl === link.href) {
            link.parentElement.classList.add('active');
        }
    });
}

document.addEventListener('DOMContentLoaded', updateActiveNavLink);